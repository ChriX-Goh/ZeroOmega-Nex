...
if text.count(old_destructure) != 1:
    raise SystemExit('proxy validator destructure anchor mismatch')
text = text.replace(old_destructure, new_destructure)

old_reads = '''  readFile(visualEvidenceWorkflowPath, 'utf8'),
  readFile(sessionCheckpointPath, 'utf8'),
]);'''
new_reads = '''  readFile(visualEvidenceWorkflowPath, 'utf8'),
...
if text.count(old_reads) != 1:
    raise SystemExit('proxy validator read anchor mismatch')
