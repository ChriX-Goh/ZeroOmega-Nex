...
const outputPath = resolve(
  process.env.ZEROOMEGA_ORIGINAL_RUNTIME_OUTPUT ?? 'original-runtime/virtual-results',
);
const userDataDir = await mkdtemp(resolve(tmpdir(), 'zeroomega-original-virtual-'));
...
try {
    ...
    for (let attempt = 0; attempt < 40; attempt += 1) {
      try {
        return await optionsPage.evaluate(
          async ({ requestMethod, requestArgs, requestNoReply }) => {
            if (requestNoReply) {
          ...
          { requestMethod: method, requestArgs: args, requestNoReply: noReply },
        );
      } catch (error) {
        lastError = error;
        await pause(100);
  ...
  for (const profile of [fixedProfile, virtualFixedProfile, virtualDirectProfile]) {
  ...
  const result = {
    ...
    profiles: {
      fixedProfile,
      virtualFixedProfile,
      virtualDirectProfile,
    },
    captures,
  };

  await writeFile(
    resolve(outputPath, 'virtual-results.json'),
