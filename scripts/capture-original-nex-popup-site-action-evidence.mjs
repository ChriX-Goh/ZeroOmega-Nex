...
function selectorsFor(implementation) {
  ...
  return {
    ...
    addRuleLabel: '[data-popup-add-current-site-label]',
    temporaryRule: '[data-popup-temporary-rule-toggle]',
    temporaryRuleIcon: '[data-popup-temporary-rule] [data-original-popup-icon]',
    temporaryRuleDomain: '[data-popup-temporary-domain]',
...
function single(metrics, role, implementation) {
  assert.equal(metrics[role].length, 1, `${implementation} ${role} metric`);
  return metrics[role][0];
}

function assertMetricEqual(originalMetric, nexMetric, field, label) {
  assert.equal(nexMetric[field], originalMetric[field], `${label} ${field}`);
}

function assertStyleEqual(originalMetric, nexMetric, fields, label) {
  for (const field of fields) {
    assertMetricEqual(originalMetric.style, nexMetric.style, field, label);
  }
}

function assertClosedSiteActionParity(originalEntry, nexEntry) {
  assert.deepEqual(
    nexEntry.textLines,
    originalEntry.textLines,
    'Nex closed site-action text must match the original surface',
  );

  const originalAdd = single(originalEntry.layoutMetrics, 'addRule', 'Original');
  const nexAdd = single(nexEntry.layoutMetrics, 'addRule', 'Nex');
  const originalTemporary = single(originalEntry.layoutMetrics, 'temporaryRule', 'Original');
  const nexTemporary = single(nexEntry.layoutMetrics, 'temporaryRule', 'Nex');

  assert.equal(nexAdd.text, 'Add condition', 'Nex Add condition row must not include the domain');
  assert.equal(
    nexTemporary.text,
    'example.com',
    'Nex temporary-rule row must show only the domain',
  );

  for (const [label, originalMetric, nexMetric] of [
    ['Add condition row', originalAdd, nexAdd],
    ['Temporary-rule row', originalTemporary, nexTemporary],
  ]) {
    for (const field of ['width', 'height']) {
      assertMetricEqual(originalMetric.rect, nexMetric.rect, field, label);
    }
    assertStyleEqual(
      originalMetric,
      nexMetric,
  ...
  assert.ok(
    nexTemporary.rect.top >= nexAdd.rect.bottom && nexTemporary.rect.top <= nexAdd.rect.bottom + 2,
    'Nex temporary-rule row must immediately follow Add condition',
  );
  ...
  for (const role of [
    ...
    assert.equal(
      nexEntry.layoutMetrics[role].length,
      originalEntry.layoutMetrics[role].length,
      `Nex ${role} count must match Original`,
    );
  }
  assert.equal(
    nexEntry.layoutMetrics.temporaryRuleSelect.length,
    0,
    'Nex must not expose a persistent temporary-rule select',
  );
}

assertClosedSiteActionParity(original, nex);
const comparison = {
