...
- `PROJECT_STATE.json` — machine-readable current authorization, progress anchor, owner result, and release state.
...
- Product completion audit anchor: 47.9%, reported as 48%.
- Confidence band: 43%–50%.
- Release state: `NO-GO`.
- Latest owner result: Firefox `FAIL`, 2026-08-02.
- Active batch: `GOV-01` — governance and compatibility-class convergence.
- Product code: frozen during `GOV-01`.
- Product progress change from `GOV-01`: zero.
- Next product batch: `MIG-01` — real original-export golden path.
...
- `CONTRACT-EXACT`: data, configuration meaning, route results, persistence, restart, semantic export, failure recovery, rollback, and security.
...
The earlier strict original-facing correction was necessary because the product exposed Nex-only engineering concepts, excessive prose, altered defaults, and unfamiliar navigation while real direct migration remained incomplete. That correction remains valid.

The new boundary prevents the correction from becoming a permanent requirement to clone every DOM node, pixel, blur/click sequence, animation, or historical defect.
...
The following engineering assets remain valuable and are not discarded by governance correction:
...
Existing Popup slices 02Q/02R/02S remain bounded regression `PASS` results. They do not close the full Toolbar/Popup parent journey and no longer control the immediate product sequence.
...
Complete the repository-wide governance convergence:

1. align core documents with the four compatibility classes;
2. bind all agents to product WIP=1, stop rules, debt budgets, and proportional evidence;
3. remove the old strict serial dependency that required complete Order 1 owner acceptance before real migration;
4. preserve the 47.9%/48% audit anchor;
5. classify remaining active audit rows without double counting bounded slices;
6. perform one explicit tri-track reconciliation after governance convergence;
7. do not change product code or claim product progress.

### `MIG-01`

After `GOV-01`, the only authorized product implementation batch is:
...
Popup/Options beautification, new diagnostics, scheduling, history, backup, remote sync, native engine, and new Profile families remain frozen during `MIG-01`.
