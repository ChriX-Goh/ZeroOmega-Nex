...
| D-05 | 条件专属字段     | `profile_switch.jade`、`switch_profile.coffee` | 专属控件允许编辑中暂时无效；Apply 严格校验 | MUST_MATCH | PARTIAL  | MISSING | 空 pattern/错误正则可作为 Draft warning 保存；Apply 与 Applied 仍严格，尚缺字段级错误文案 | 补 locale 与内联错误  |
