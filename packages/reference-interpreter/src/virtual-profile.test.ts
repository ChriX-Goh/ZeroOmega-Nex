...
it('resolves a Virtual profile through its selected target', () => {
  const decision = evaluateProfileGraph(
    virtualSpec(),
    { kind: 'profile', profileId: 'profile-virtual' },
    { url: 'https://example.test/', host: 'example.test', scheme: 'https' },
  );
  expect(decision).toMatchObject({ status: 'resolved', route: { kind: 'proxy' } });
  expect(decision.trace.some((entry) => entry.action === 'virtual')).toBe(true);
});
