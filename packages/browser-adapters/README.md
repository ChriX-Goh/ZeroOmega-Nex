...
The built extension declares the authentication permissions and HTTP(S) origins as optional. Startup checks existing grants but never calls the permission-request API; permission prompts remain an explicit future user action in the profile workflow.
